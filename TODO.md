...
- [ ] **No-account (anonymous) path still unverified live: your installed Netlify CLI is too old for `--allow-anonymous`** (the flag shipped ~2026-03). The tool detects this and guides instead of erroring. To exercise it: `npm i -g netlify-cli@latest`, then `tl deploy` with no token set.
