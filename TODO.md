...
## Backend-capable build + deploy
- [x] **Backend-capable BUILD (done, 0.5.0, in tree).** When the three-question backend test says a product genuinely needs a server, `/tl-build` now emits a real backend alongside the static front end: serverless functions under `netlify/functions/` (one per backend R-ID, glossary-named), a `schema.sql`, a names-only `.env.example` (with `!.env.example` un-ignored in the project `.gitignore`), an updated `netlify.toml` (a `[functions]` directory), and a `BACKEND.md` deploy guide, recorded in a forward-looking `backend` block in `build.json`. Neon Postgres is the documented default (`@neondatabase/serverless`, `DATABASE_URL`/`NETLIFY_DATABASE_URL`, where Netlify DB is managed Neon), overridable to any Postgres. New `core/backend.ts` holds the deterministic names-only `.env.example` and `BACKEND.md` generators, unit-tested for the names-only (`^[A-Z0-9_]+=$`) and no-dash invariants. Static stays the gated default; deploy publishes the static front end as before, with an actionable message pointing at `BACKEND.md`.
- [ ] **Publish `@hobocode/thought-layer@0.5.0`** (the backend-capable build; version already bumped to 0.5.0 in the tree). `npm publish` from your own terminal (passkey 2FA, Claude can't). `prepublishOnly` runs typecheck + test + build.
- [ ] **Backend DEPLOY automation (the follow-up to this build pass).** Teach `tl deploy` to ship the functions and set the env vars into the user's own Netlify account (and provision or connect Neon), reading the `build.json` `backend` block. Today `tl deploy` still publishes only the static front end and points the user at `BACKEND.md` for the one-time manual steps. Still BYOK, owned, nothing phones home.
- [ ] **Local script building (utilities + workflows) as a build target.** The sibling of the hosted-backend track: when the rigor validates an idea best shipped as local scripts, CLI utilities, or automation workflows (not a hosted web app), the build emits and installs those locally instead of deploying to Netlify. Deferred ("we'll do that later").
- [ ] Also deferred: Cloudflare/Vercel deploy adapters, and the documented full-server or container host path (Render, Fly, Coolify) for products that need more than serverless functions.
